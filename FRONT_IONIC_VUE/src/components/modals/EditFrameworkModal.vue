            ...
            >
              <DialogPanel class="flex w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 shadow-xl transition-all">
                ...
                <form @submit.prevent="submitForm" class="mx-auto w-full">
                  ...
                  <div class="mb-5 ">
                    <label for="level" class="block mb-2 text-sm font-medium text-gray-900">Nivel</label>
                    <input type="text" id="level" v-model="formData.level" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 ">
                  </div>
                  <div class="mb-5 ">
                    <label for="year" class="block mb-2 text-sm font-medium text-gray-900">Año</label>
                    <input type="text" id="year" v-model="formData.year" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 ">
                  </div>
...
import { updateFramework } from '@/backend/info';
...
const formData = ref<FrameworkForm>({
  name: '',
  level: '',
  year: ''
});
...
async function submitForm(): Promise<void> {
  try {
    if(props.id){
      await updateFramework(formData.value, props.id);
